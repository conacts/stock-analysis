#!/usr/bin/env python3
"""
Test All Trigger.dev Endpoints

Comprehensive test script to verify all production API endpoints and
Trigger.dev tasks are working correctly. This script tests:

1. All Railway API endpoints
2. AI Trading system endpoints
3. Portfolio analysis functionality
4. Market data and risk management
5. Trigger.dev task compatibility

Run this to verify the entire system is working in production.
"""
...
async def test_health_endpoint():
    """Test the health check endpoint with 30-second timeout."""
    try:
...
async def test_portfolio_endpoints():
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, headers=headers) as session:
            ...
            async with session.post(f"{RAILWAY_URL}/portfolio/analyze-with-llm", headers=headers, json=llm_payload) as response:
                if response.status == 200:
                    data = await response.json()
...
async def test_trading_endpoints():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            # Test trading config
            async with session.get(f"{RAILWAY_URL}/trading/trading-config", headers=headers) as response:
                if response.status == 200:
            ...
            # Test market status
            async with session.get(f"{RAILWAY_URL}/trading/market-status", headers=headers) as response:
                if response.status == 200:
            ...
            # Test risk status
            async with session.get(f"{RAILWAY_URL}/trading/risk-status/1", headers=headers) as response:
                if response.status == 200:
...
async def test_analysis_endpoints():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(f"{RAILWAY_URL}/analysis/market-gaps", headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
            ...
            async with session.post(f"{RAILWAY_URL}/analysis/daily-performance", headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
...
async def test_alert_endpoints():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(f"{RAILWAY_URL}/alerts/price-alerts/active", headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
            ...
            async with session.post(f"{RAILWAY_URL}/alerts/opening-bell", headers=headers, json=alert_payload) as response:
                if response.status == 200:
                    data = await response.json()
...
async def test_news_endpoints():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(f"{RAILWAY_URL}/news/overnight-analysis", headers=headers, json=news_payload) as response:
                if response.status == 200:
                    data = await response.json()
...
async def test_trigger_compatibility():
    ...
    print("\n🔧 Testing Trigger.dev Compatibility")
    print("-" * 40)

    # Test portfolio analysis payload structure
    portfolio_data = {"portfolioId": 1, "portfolioName": "Test Portfolio", "analysisType": "comprehensive", "includeLLMHistory": False}

    # Simulate what Trigger.dev would send
    headers = {"Authorization": f"Bearer {API_TOKEN}"}

    try:
        async with aiohttp.ClientSession() as session:
            # Test portfolio summary structure
            async with session.get(f"{RAILWAY_URL}/portfolio/1/summary", headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
            ...
            llm_payload = {"portfolio_data": portfolio_data, "analysis_type": "comprehensive"}
...
async def main():
    """Run all tests."""
    print("🧪 COMPREHENSIVE PRODUCTION API TEST")
    print("=" * 60)
