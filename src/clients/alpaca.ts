...
export class AlpacaClient {
	...
	constructor(
		...
		this.apiKey = apiKey || config_instance.alpaca_api_key || '';
		this.secretKey = secretKey || config_instance.alpaca_secret_key || '';
		this.baseUrl = baseUrl || config_instance.alpaca_base_url ||
			(isPaper ? 'https://paper-api.alpaca.markets' : 'https://api.alpaca.markets');

		if (!this.apiKey || !this.secretKey) {
			throw new Error('Alpaca API key and secret key are required. Set ALPACA_API_KEY and ALPACA_SECRET_KEY environment variables.');
		}
	...
	public async replaceOrder(orderId: string, orderRequest: Partial<OrderRequest>): Promise<AlpacaOrder> {
	...
	// Convenience Methods
	public async buyMarket(symbol: string, qty: number, extendedHours: boolean = false): Promise<AlpacaOrder> {
	...
	public async sellMarket(symbol: string, qty: number, extendedHours: boolean = false): Promise<AlpacaOrder> {
		return this.createOrder({
			symbol,
			qty,
			side: 'sell',
