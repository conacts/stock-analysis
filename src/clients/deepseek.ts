...
export class DeepSeekClient {
    ...
    const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
      {
        role: 'system',
        content:
          'You are an expert financial analyst. Provide comprehensive stock analysis with specific recommendations.',
      },
      {
        role: 'user',
        content: prompt,
    ...
    const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
      {
        role: 'system',
        content:
          'You are a financial news analyst. Analyze news impact on stock prices with specific metrics.',
      },
      {
        role: 'user',
        content: prompt,
    ...
    const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
      {
        role: 'system',
        content:
          'You are a growth investment analyst. Identify key catalysts that could drive stock price appreciation.',
      },
      {
        role: 'user',
        content: prompt,
  ...
  public getCostSummary(): DeepSeekCostSummary {
    return {
      total_input_tokens: this.totalTokensUsed,
      total_output_tokens: this.totalTokensUsed,
      total_tokens: this.totalTokensUsed,
      input_cost_usd: this.totalCost,
      output_cost_usd: this.totalCost,
      total_cost_usd: this.totalCost,
      requests_made: this.apiCallsMade,
    };
  }
  ...
  private async makeApiCall(
    ...
    try {
      ...
      this.apiCallsMade++;
      if (response.usage) {
        this.totalTokensUsed += response.usage.prompt_tokens + response.usage.completion_tokens;
        // DeepSeek pricing: ~$0.14 per 1M input tokens, ~$0.28 per 1M output tokens
        const inputCost = (response.usage.prompt_tokens / 1000000) * 0.27;
        const outputCost = (response.usage.completion_tokens / 1000000) * 1.1;
