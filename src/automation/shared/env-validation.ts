...
function getApiUrl(): string {
	...
	// Use explicit PYTHON_API_URL if set
	...
	if (isProduction) {
		return 'https://stock-analysis-production-31e9.up.railway.app';
	} else {
...
export function validateRequiredEnvVars(requiredVars: (keyof RequiredEnvVars)[] = ['PYTHON_API_URL', 'API_TOKEN']): RequiredEnvVars {
	...
	// Handle PYTHON_API_URL with auto-detection
	if (requiredVars.includes('PYTHON_API_URL')) {
		envVars.PYTHON_API_URL = getApiUrl();
	}

	// Check other required variables
	for (const varName of requiredVars) {
		if (varName === 'PYTHON_API_URL') continue; // Already handled above
